    ...
    var body: some View {
            ...
            .onAppear {
                if isRunningInPreview {
                    showAuthSheet = false
                    // Optional: Mock-Posts für eine „echte“ Vorschau
            ...
            .onDisappear {
                if !isRunningInPreview { stopListeningPosts() }
            }
            .task {
                if !isRunningInPreview { await userVM.loadProfile() }
            }
            .fullScreenCover(isPresented: $showAuthSheet) {
                // In Preview bleibt showAuthSheet == false, also wird das hier nicht gezeigt
